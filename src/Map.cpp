...
//--------------------------------------------------------------------------------
// Function    : Map::Initialize_Selection_Button
// Description : Initialize Selection_Button classes
// Note        :
// Input       : None
// Output      : None
//--------------------------------------------------------------------------------
void Map::Initialize_Selection_Button(Selection_Button& Button,const int x,const int y,const int mode, string image_filename){
    Button.set_value(x,y,mode);
    Button.load_soundtrack();
    Button.loadFromFile(image_filename); 
}// FUNCTION : Map::Initialize_Selection_Button
    ...
    switch(mode){
        ...
        case MAIN_MENU_MODE:
            for(int i=0;i<menu_num-2;i++){
                ...
                Change_Mode(MAIN_MENU_MODE,maple,NULL);
            }//for(int i=0;i<menu_num-2;i++)
        ...
        case BIG_MAP_MODE:
            ...
            Enter_Battle_Scenario(b,Scenario_Num);
        ...
        case BATTLE_SCENARIO_MODE:
            ...
            //Leave_Battle_Scenario(b,Scenario_Num,Last_Battle,Enforce_Battle_Deletion);
        ...
        case TUTORIAL_MODE:
            ...
            Map_Button[gomenu].map_button_reaction(e,mode);
            Change_Mode(TUTORIAL_MODE,bar,NULL); 
        ...
        case GAME_INFORMATION_MODE:
            Map_Button[gomenu].map_button_reaction(e,mode);
            Change_Mode(MAIN_MENU_MODE,-1,&Game_Info[0]);             
        ...
        case BACKGROUND_INTRO_MODE://
            ...
            Change_Mode(BACKGROUND_INTRO_MODE,teacher1,&Scrolling_BG[1]);             
        ...
        case FINISH_SCENE_MODE:
            for(int i=3;i<menu_num;i++){
                Menu_Button[i].menu_button_reaction(e,mode);
                Change_Mode(FINISH_SCENE_MODE,sorrow,NULL);        
            }//for(int i=3;i<menu_num;i++)
...
void Map::Change_Mode(const int mode_in,const int bgm_name,Change_scene* Scene){
    if(mode!=mode_in){
...
void Map::Enter_Battle_Scenario(BATTLE_SCENE **b, const int scenario_num){
    if(mode!=BIG_MAP_MODE){
        played[scenario_num] = 1;
        #ifdef DEBUG
            Delete_Battles(Enforce_Battle_Deletion,b,Last_Battle);
        #endif
...
void Map::Leave_Battle_Scenario(BATTLE_SCENE **b, int& scenario_num,int& last_battle,bool& enforce){
    if(mode!=BATTLE_SCENARIO_MODE){
        ...
        last_battle = scenario_num;
        enforce = true;

        scenario_num++;//Must notice the order
    }//if(mode==BIG_MAP_MODE)
}

void Map::Delete_Battles(bool& enforce,BATTLE_SCENE **b,const int last_battle){
    if(enforce){
        //delete b[last_battle];
        //b[last_battle]=NULL;
        #ifdef DEBUG
            //printf("%d\n",last_battle);
        #endif
